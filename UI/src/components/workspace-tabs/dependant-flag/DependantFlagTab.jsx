...
const DependentFlagForm = lazy(() => import('./DependentFlagForm'));
const UpdateDependentFlagModal = lazy(() => import('./UpdateDependentFlagModal'));
...
const DependantFlagTab = ({ env, envId }) => {
    ...
    const displayFlags = depFlags.length === 0 && fetchedFlags && fetchedFlags.length > 0 ? fetchedFlags : depFlags;
    ...
    const existingKeys = useMemo(() => new Set([...displayFlags.map(f => f.key), ...allCoreFlags.map(f => f.key)]), [displayFlags, allCoreFlags]);
                    ...
                    <div style={{ flex: 1, overflow: 'hidden' }}>
                        <Suspense fallback={
                            <div className="graph-empty-state">
                                <i className="ri-loader-4-line spin" style={{ fontSize: '32px', color: '#a78bfa', opacity: 0.6 }}></i>
