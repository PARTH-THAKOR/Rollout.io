...
const CreateEnvironmentModal = ({ onClose, onSubmit, serverError = '', isSubmitting = false, onNameChange }) => {
    const [newEnvName, setNewEnvName] = useState('');
    const ENV_MAX = 50;
    const envLen = newEnvName.length;
    const envCountClass = envLen >= ENV_MAX ? 'error' : envLen >= 40 ? 'warning' : '';

    const handleSubmit = (e) => {
        e.preventDefault();
        if (newEnvName.trim() && envLen <= ENV_MAX && !isSubmitting) {
            onSubmit(newEnvName.trim());
        }
    };

    return (
        <div className="modal-overlay" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000, animation: 'fadeIn 0.2s ease-out' }} onClick={() => !isSubmitting && onClose()}>
            <div className="modal-content glass-card" onClick={(e) => e.stopPropagation()} style={{ background: 'linear-gradient(180deg, rgba(20,10,40,0.95), rgba(10,5,20,0.98))', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', width: '400px', padding: '28px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.8)', animation: 'slideUp 0.3s cubic-bezier(0.4, 0, 0.2, 1)', position: 'relative' }}>
                ...
                <form onSubmit={handleSubmit}>
                    <div style={{ marginBottom: '24px' }}>
                        <label className="form-label">
                            Environment Name <span className="form-required">*</span>
                        </label>
                        <div style={{ position: 'relative' }}>
                            <input
                                ...
                                className={`login-input ${envLen === 0 ? '' : envLen > ENV_MAX ? 'input-error' : ''}`}
                                style={{ width: '100%', boxSizing: 'border-box' }}
                                maxLength={ENV_MAX + 5}
                                disabled={isSubmitting}
                            />
                            {envLen > 0 && (
                                <div className={`char-count ${envCountClass}`} style={{ textAlign: 'right', marginTop: '4px' }}>
                                    {envLen}/{ENV_MAX}
                                </div>
                            )}
                        </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                        ...
                        <button type="submit" className="btn-primary" disabled={!newEnvName.trim() || envLen > ENV_MAX || isSubmitting} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                            {isSubmitting ? (
                                <><i className="ri-loader-4-line spin" style={{ fontSize: '14px' }}></i> Creating...</>
                            ) : (
                                'Create'
                            )}
                        </button>
